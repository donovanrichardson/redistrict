#!/usr/bin/env python3
"""
Create (or extend) counties_2020 by aggregating tracts_2020.

Each county row contains:
  - geoid20      TEXT PK  (5-char: LEFT(geoid20, 5))
  - statefp20    TEXT
  - countyfp20   TEXT
  - geom         MultiPolygon(4269) via ST_Union of tract geometries
  - pop20        INTEGER  sum of tract pop20
  - aland20      NUMERIC  sum of tract aland20
  - awater20     NUMERIC  sum of tract awater20
...
Goal
----
Counties are the coarsest geography used by the redistricting algorithm.
They are derived by aggregating tracts_2020, preserving population-weighted
centroids so each county centroid reflects where people actually live.

Processing
----------
By default, processes all states that are 'done' in tracts_state_log but not
yet recorded in counties_state_log, in FIPS order. Override with STATE_FIPS
env var (comma-separated) to target specific states. Each state is one
transaction. On failure the data is rolled back, a 'failed' row is written to
counties_state_log, and the script moves on — that state will not be retried
on subsequent runs unless its log row is deleted or STATE_FIPS override is used.
"""
...
CREATE_TABLE_SQL = """
CREATE TABLE IF NOT EXISTS public.counties_state_log (
...
CREATE TABLE IF NOT EXISTS public.counties_2020 (
...
CREATE INDEX IF NOT EXISTS counties_2020_geom_idx
    ON public.counties_2020 USING GIST (geom);
CREATE INDEX IF NOT EXISTS counties_2020_centroid_idx
    ON public.counties_2020 USING GIST (centroid_geom);
CREATE INDEX IF NOT EXISTS counties_2020_statefp_idx
    ON public.counties_2020 (statefp20);
"""

# States done in tracts_state_log but not yet in counties_state_log.
PENDING_STATES_SQL = """
SELECT l.statefp20
FROM public.tracts_state_log l
WHERE l.status = 'done'
  AND NOT EXISTS (
      SELECT 1 FROM public.counties_state_log cl
      WHERE cl.statefp20 = l.statefp20
  )
ORDER BY l.statefp20;
"""

LOG_STATE_SQL = """
INSERT INTO public.counties_state_log (statefp20, status, updated_at, error_msg)
...
UPSERT_COUNTY_SQL = """
INSERT INTO public.counties_2020
    (geoid20, statefp20, countyfp20,
     geom, pop20, aland20, awater20,
     intptlat20, intptlon20, centroid_geom)
WITH agg AS (
    SELECT
        LEFT(geoid20, 5)                           AS geoid20,
        statefp20,
        countyfp20,
    ...
    GROUP BY LEFT(geoid20, 5), statefp20, countyfp20
)
SELECT
    geoid20,
    statefp20,
...
def setup_logging() -> logging.Logger:
    ...
    log_path = logs_dir / f"create_counties_{timestamp}.log"

    logger = logging.getLogger("create_counties")
...
def get_pending_states(conn) -> list[str]:
    """Return states done in tracts_state_log but not yet in counties_state_log."""
...
def process_state(cur, statefp: str, log: logging.Logger) -> int:
    """Upsert all counties for one state. Returns row count."""
    cur.execute(COUNTIES_SQL, {"statefp": statefp})
    counties = [row[0] for row in cur.fetchall()]
    if not counties:
        log.warning(f"State {statefp}: no counties found in tracts_2020, skipping.")
        return 0
...
def main():
    ...
    try:
        with conn.cursor() as cur:
            log.info("Ensuring counties_2020 table and indexes exist...")
        ...
        if not states_to_run:
            log.info("Nothing to do — all eligible states already present in counties_2020.")
            return
        ...
        with conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM public.counties_2020")
            total = cur.fetchone()[0]
            cur.execute("SELECT COUNT(*) FROM public.counties_2020 WHERE pop20 = 0")
            zero_pop = cur.fetchone()[0]
            cur.execute("SELECT COUNT(DISTINCT statefp20) FROM public.counties_2020")
